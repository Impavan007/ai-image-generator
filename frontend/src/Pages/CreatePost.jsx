...
const CreatePost = () => {
    ...
    const handleSubmit=async (e)=>{
        ...
        if(form.prompt && form.photo){
            setLoading(true)
            try {
                const response= await fetch('https://mern-ai-image-generator-s2f0.onrender.com/api/v1/post',{
                    method:'POST',
                    headers:{
    ...
    const generateImage= async()=>{
        if(form.prompt && form.name){
            ...
            try {
                ...
                const response = await fetch('https://mern-ai-image-generator-s2f0.onrender.com/api/v1/dalle',{
                    method:'POST',
                    headers:{
                        'Content-Type':'application/json'
                    },
